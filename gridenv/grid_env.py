# 这是一个环境的类，不需要考虑可视化的东西，管理这个环境的所有参数，可视化的东西通过render。py进行实现。
# 可视化的东西通过self.render_ 方法进行调用。
# self.render_ = render.Render(target=target, forbidden=forbidden, size=size)
# 这样就把基本的网格世界初始化界面可以画出来了
import time
...
class GridEnv(gym.Env):
    ...
    def psa_rsa_init(self):
        ...
        self.Rsa = np.zeros(shape=(self.size ** 2, self.action_space_size,
                                   len(self.reward_list)), dtype=float)
        for state_index in range(state_size):
            for action_index in range(self.action_space_size):
                ...
                if next_pos[0] < 0 or next_pos[1] < 0 or next_pos[0] > \
                        self.size - 1 or next_pos[1] > self.size - 1:
...
if __name__ == "__main__":
    grid = GridEnv(size=5, target=[1, 2], forbidden=[[2, 2]], render_mode='')  # 对于size,target,forbidden,是否有滑动现象进行初始化
    # 就是初始化网格世界，现在gym库不再维护了，换做Gymnasium库，import gymnasium as gym
    grid.render()
